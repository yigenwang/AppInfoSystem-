...
$(".modifyVersion").on("click",function(){
	...
	if(status=="1"||status=="3"){//只有待审核状态或审核不通过的情况下才能进行修改
		if(versionid==""||versionid==""){
			alert("APP应用下无版本信息，请先增加版本新信息！");
		}else{
			window.location.href="appversionmodify?vid="+versionid+"&aid="+appinfoid;
		}
	}else{
		alert("该APP应用的状态为：【"+obj.attr("statusname")+"】,不能修改其他版本，只能进行【新增版本】操作");
	}
});
			...
			if(data.errorCode=="0"){
				if(data.resultMsg=="success"){
					...
					if("open"==obj.attr("saleSwitch")){
						...
						alert("dddd【"+obj.attr("appsoftwarename")+"】的【下架】	操作失败！");
					}
				}
			}else if(data.errorCode=="exception000001"){
